...
public class FindAllRoute {
    ...
    public FindAllRoute(Project project,int model) {
        this.project = project;
        //从数据库中查询出所有工序的节点及工期信息
        nodePeriodList.addAll(new DrawMetaDAO().findAllNodeAndPeriod(project,model));
        //找出所有开始节点相应的结束节点
        findEndsForStart();
        ...
        while (!sourceAndCurrentStack.empty()) {
            ...
            if (null != targetsForSource.get(current)) {
                ...
                for (EndAndPeriod tp : targets) {
                    sourceAndCurrentStack.push(new NodeAndPeriod(current, tp.getEnd(), tp.getPeriod()));
                }
            }
            if (!sourceAndCurrentStack.empty() && null == targetsForSource.get(current)) {
                ...
                currentRoute = currentRoute.copy(currentRoute.getId() + 1, sourceAndCurrentStack.peek().getStart());
            }
        }
            ...
            if (route.getTotalPeriod() == maxTotalPeriod) {
                ...
                try {
                    ...
                    for (int i = 0; i < nodelist.size() - 1; i++) {
                        drawMetaDAO.updateCoordinate("true", nodelist.get(i).getEnd(), nodelist.get(i + 1).getEnd());
                    }
                    tx.commit();
    ...
    private void findEndsForStart() {
        for (NodeAndPeriod np : nodePeriodList) {
            ...
            if (targetsForSource.containsKey(np.getStart())) {
                //取得np的结束节点集合
                List<EndAndPeriod> targets = targetsForSource.get(np.getStart());
