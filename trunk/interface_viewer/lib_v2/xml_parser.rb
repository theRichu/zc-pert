class XMLParser
  ...
  def initialize(xml_path, tree_fields, item_position)
    @xml_path = xml_path
  ...
  def pre_parse()
    ...
    parent_item_list, child_elem_list, parent_elem_list = [], [], []
    # add root item
    ...
    @item_position.add(@xml_path, root_elem.path, root_item, pos_start, pos_end)
    # initialize the arrays
    root_elem.children.each do |elem|
      if elem.elem? then
        parent_item_list.push(root_item)
        parent_elem_list.push(root_elem)
        child_elem_list.push(elem)
      end
    end
    return parent_item_list, child_elem_list, parent_elem_list
  end
  ...
  def parse()
    ...
    parent_elem_list = condition_list[2]
    form_tree(parent_item_list, child_elem_list, parent_elem_list)
  end
  
  def form_tree(parent_item_list, child_elem_list, parent_elem_list)
    ...
    until child_elem_list.empty?
      ...
      parent_elem = parent_elem_list.shift
      ...
      current_elem.children.each do |elem|
        if elem.elem? then
          ...
          parent_elem_list.push(current_elem)
          child_elem_list.push(elem)
          has_child = true
        end
      end
      set_item_data(parent_item, parent_elem, current_item, current_elem) 
    end
  end
  
  def set_item_data(parent_item, parent_elem, current_item, current_elem)
    parent_pos = @item_position.get_pos(@xml_path + "|" + parent_elem.path)
    ...
    if parent_pos && parent_content then
      ...
      @item_position.add(@xml_path, current_elem.path, current_item, child_start, child_end)
    end
